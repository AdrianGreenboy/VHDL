-- =============================================================================
--  mem_subsys_usart.vhd  -  Subsistema de memoria con motor DMA (bursts)
--  Licencia: MIT
--
--  Calca de mem_subsys_spi con la region del USART en vez de la del SPI:
--    region 0x0000_0000 (bits 31:30 = "00")   -> RAM local, 1 ciclo
--    region 0x4000_0000 (bits 31:28 = "0100") -> registros del DMA, 1 ciclo
--    region 0x6000_0000 (bits 31:28 = "0110") -> IP USART (usart_axi_top,
...
entity mem_subsys_usart is
  ...
  port (
    ...
    -- region USART (0x6000_0000, bits 31:28 = "0110")
    usart_sel   : out std_logic;
    usart_addr  : out std_logic_vector(7 downto 0);
    usart_rdata : in  word_t;
...
end entity mem_subsys_usart;

architecture rtl of mem_subsys_usart is
  signal is_local, is_dmareg, is_usart : std_logic;
...
begin
  ...
  is_usart  <= '1' when dmem_addr(31 downto 28) = "0110" else '0';

  usart_sel  <= is_usart;
  usart_addr <= dmem_addr(7 downto 0);
  ...
  dmem_rdata <= loc_rdata    when is_local  = '1' else
                ...
                usart_rdata  when is_usart  = '1' else
                (others => '0');
  dmem_ready <= '1';
