...
#
#  RV32IMA + Zicsr. Extendido desde el asm.py RV32IM original.
...
CSR_NAMES = {"mstatus":0x300,"misa":0x301,"mie":0x304,"mtvec":0x305,
             ...
             "mip":0x344,"mcycle":0xB00,"mcycleh":0xB80,
             "mvendorid":0xF11,"marchid":0xF12,"mimpid":0xF13,"mhartid":0xF14}

# Extension A: name -> funct5 (opcode 0x2F, funct3=010 para .w)
ATYPE = {"amoadd.w":0x00,"amoswap.w":0x01,"lr.w":0x02,"sc.w":0x03,
         "amoxor.w":0x04,"amoor.w":0x08,"amoand.w":0x0C,
         "amomin.w":0x10,"amomax.w":0x14,"amominu.w":0x18,"amomaxu.w":0x1C}

# loads estrechos: name -> funct3 (opcode 0x03)
LOADS = {"lb":0,"lh":1,"lw":2,"lbu":4,"lhu":5}
# stores estrechos: name -> funct3 (opcode 0x23)
STORES = {"sb":0,"sh":1,"sw":2}

def enc_amo(f5, aq, rl, rs2, rs1, rd):
    return (f5<<27)|(aq<<26)|(rl<<25)|(rs2<<20)|(rs1<<15)|(2<<12)|(rd<<7)|0x2F
...
def assemble(src):
    ...
    for pc, l in prog:
        ...
        if op in RTYPE:
            ...
            words.append(enc_i(imm, rs1, LOADS[op], reg(a[0]), 0x03))
        elif op in STORES:
            imm, rs1 = parse_mem_operand(a[1])
            words.append(enc_s(imm, reg(a[0]), rs1, STORES[op], 0x23))
        elif op in ATYPE:
            f5 = ATYPE[op]
            if op == "lr.w":            # lr.w rd, (rs1)
                inner = a[1].strip().strip("()")
                rs1 = reg(inner)
                words.append(enc_amo(f5, 0, 0, 0, rs1, reg(a[0])))
            else:                        # amo*/sc.w rd, rs2, (rs1)
                inner = a[2].strip().strip("()")
                rs1 = reg(inner)
                words.append(enc_amo(f5, 0, 0, reg(a[1]), rs1, reg(a[0])))
        elif op in BTYPE:
            f3 = BTYPE[op]
        ...
        elif op == "wfi":    words.append(0x10500073)
        elif op == "fence":  words.append(0x0FF0000F)
        elif op == "fence.i":words.append(0x0000100F)
        else:
