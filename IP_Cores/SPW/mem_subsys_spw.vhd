-- =============================================================================
--  mem_subsys_spw.vhd  -  Subsistema de memoria con motor DMA (bursts)
--  Licencia: MIT
--
--  Calca de mem_subsys_can con la region del SpaceWire (0xB000_0000) en vez
--  de la del CAN:
--    region 0x0000_0000 (bits 31:30 = "00")   -> RAM local, 1 ciclo
--    region 0x4000_0000 (bits 31:28 = "0100") -> registros del DMA, 1 ciclo
--    region 0xB000_0000 (bits 31:28 = "1011") -> IP SpaceWire (spw_mmio,
--                                                instanciado en el top; aqui
--                                                solo decode + ruteo de rdata)
--
--  NOTA de region: 0xB000_0000 es espacio del bus dmem INTERNO del RV32,
--  no de la DDR fisica — no colisiona con el buffer DDR reservado del PS.
--
...
entity mem_subsys_spw is
  ...
  port (
    ...
    -- region SpaceWire (0xB000_0000, bits 31:28 = "1011")
    spw_sel   : out std_logic;
    spw_addr  : out std_logic_vector(7 downto 0);
    spw_rdata : in  word_t;
...
end entity mem_subsys_spw;

architecture rtl of mem_subsys_spw is
  signal is_local, is_dmareg, is_spw : std_logic;
...
begin
  ...
  is_spw    <= '1' when dmem_addr(31 downto 28) = "1011" else '0';

  spw_sel  <= is_spw;
  spw_addr <= dmem_addr(7 downto 0);
