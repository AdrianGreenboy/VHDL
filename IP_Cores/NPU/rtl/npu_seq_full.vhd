...
architecture rtl of npu_seq_full is
  ...
  type t_state is (S_IDLE,
                   ...
                   S_C2_RED, S_C2_ACC, S_C2_NEXT, S_C2_RQRD, S_C2_RQ,
                   S_P2, S_FC_RD, S_FC_MAC, S_FC_RQ, S_ARGMAX, S_DONE);
  ...
  -- Etapa de lectura de la FC: separar la lectura de memoria del producto
  -- rompe la cadena contador -> LUTRAM -> DSP -> acarreo de 32 bits que
  -- generaba 40 niveles logicos (WNS -1.924 ns en la sintesis OOC).
  signal fc_fv  : signed(C_DATA_W-1 downto 0) := (others => '0');
  signal fc_wv  : signed(C_DATA_W-1 downto 0) := (others => '0');
  -- Registro intermedio del requantize de conv2: separa la lectura de accm
  -- del requantize y la escritura en c2_ram (caminos de -0.198 ns en OOC).
  signal rq_val : t_acc_arr(0 to 7) := (others => (others => '0'));
    ...
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        case state is
          ...
          when S_C2_RQRD =>
            -- Etapa 1: lectura registrada de los 8 acumuladores del pixel
            if rq_i < 64 then
              for k in 0 to 7 loop
                rq_val(k) <= to_signed(accm(k)(rq_i), C_ACC_W);
              end loop;
              state <= S_C2_RQ;
            else
              if tile = 1 then
          ...
          when S_C2_RQ =>
            -- Etapa 2: requantize y escritura, con los operandos registrados
            if rq_i < 64 then
              for k in 0 to 7 loop
                och := tile*8 + k;
                c2_ram(och*64 + rq_i) <= to_integer(
                  do_requant(rq_val(k), mult2_in, true));
              end loop;
              rq_i  <= rq_i + 1;
              state <= S_C2_RQRD;
            else
          ...
          when S_FC_RD =>
            -- Etapa 1: solo lectura de memorias, registrada
            fc_fv <= to_signed(p2_ram(fc_i), C_DATA_W);
            if G_MUT = 3 then
              fc_wv <= to_signed(w3_ram(fc_i*10 + fc_o), C_DATA_W);  -- MUT 3
            else
              fc_wv <= to_signed(w3_ram(fc_o*256 + fc_i), C_DATA_W);
            end if;
            state <= S_FC_MAC;

          when S_FC_MAC =>
            -- Etapa 2: producto y acumulacion, con los operandos ya registrados
            sumv := resize(fc_acc, C_ACC_W+1)
                  + resize(fc_fv * fc_wv, C_ACC_W+1);
          ...
          when S_FC_RQ =>
            -- Etapa 3: requantize del logit, fuera del bucle de acumulacion
            lg_ram(fc_o) <= to_integer(do_requant(fc_res(fc_o), mult3_in, false));
            if fc_o = 9 then
              am_i    <= 1;
              am_best <= 0;
              state   <= S_ARGMAX;
