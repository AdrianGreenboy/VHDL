...
architecture rtl of npu_seq_full is
  ...
  type t_state is (S_IDLE,
                   S_C2_PIX, S_C2_MAC, S_C2_WEND, S_C2_RED, S_C2_RQ, S_C2_NEXT,
    ...
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        case state is
          ...
          when S_C2_PIX =>
            -- Bias del TILE actual: canales tile*8 .. tile*8+7.
            -- Recargarlo por tile es obligatorio (bias distintos por canal).
            for o in 0 to 7 loop
              if G_MUT = 1 then
                arr_bias(o) <= to_signed(b2_ram(o), C_ACC_W);   -- MUT 1: siempre tile 0
              else
                arr_bias(o) <= to_signed(b2_ram(tile*8 + o), C_ACC_W);
              end if;
            end loop;
            arr_win_start <= '1';
          ...
          when S_C2_MAC =>
            ...
            -- 8 canales de entrada, uno por fila del array
            ...
            -- w_mat(k*8 + r): canal de salida (tile*8+k), canal de entrada r
            for k in 0 to 7 loop
              ...
              for r in 0 to 7 loop
                if G_MUT = 2 then
                  ...
                  arr_w_mat(k*8 + r) <=
                    to_signed(w2_ram((och*8 + (7-r))*9 + kstep), C_DATA_W);
                else
                  arr_w_mat(k*8 + r) <=
                    to_signed(w2_ram((och*8 + r)*9 + kstep), C_DATA_W);
                end if;
              end loop;
            end loop;
          ...
          when S_C2_RQ =>
            for k in 0 to 7 loop
              och := tile*8 + k;
              c2_ram(och*64 + py*8 + px) <=
                to_integer(do_requant(arr_acc(k), mult2_in, true));
            end loop;
            state <= S_C2_NEXT;
