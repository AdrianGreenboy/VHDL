-- =============================================================================
--  tb_irq_pipeline.vhd  -  Interrupcion de timer (CLINT) en el core con pipeline
--  Licencia: MIT
--  Corre program_irq.mem con el CLINT mapeado en memoria y verifica que la
--  interrupcion asincrona se toma con precision en el pipeline.
-- =============================================================================
library ieee;
...
entity tb_irq_pipeline is
end entity tb_irq_pipeline;

architecture sim of tb_irq_pipeline is
  ...
  signal sel_clint, clint_we : std_logic;
...
begin
  ...
  ram_wstrb  <= "0000" when sel_clint = '1' else dmem_wstrb;
  ...
  u_clint : entity work.clint
    port map (clk => clk, rst => rst, sel => sel_clint, we => clint_we,
              addr => dmem_addr(15 downto 0), wdata => dmem_wdata,
              rdata => clint_rdata, timer_irq => timer_irq, soft_irq => soft_irq);
  ...
  begin
    ...
    wait until rising_edge(clk);
    rst <= '0';
    for i in 0 to 999 loop wait until rising_edge(clk); end loop;

    check_reg(20, x"000000C8", "x20 = 200 (lazo completo)");
    check_reg(21, x"0000ABCD", "x21 = sentinela (handler)");
    check_reg(22, x"000000C8", "x22 = 200");
    check_reg(28, x"80000007", "x28 = mcause = timer irq");

    report "-----------------------------------------";
    if errors = 0 then
      report "TODOS LOS TESTS DE INTERRUPCIONES (PIPELINE) PASARON" severity note;
    else
