-- =============================================================================
--  tb_trap.vhd  -  Testbench de CSRs + traps (ECALL / MRET) en el core
--  Licencia: MIT
--
--  Corre program_trap.mem: configura mtvec, ejecuta un ECALL, el manejador lee
--  mcause y mepc, avanza mepc y retorna con MRET. Verifica el estado final.
-- =============================================================================
library ieee;
...
architecture sim of tb_trap is
...
begin
  ...
  begin
    ...
    check_reg( 6, x"0000006F", "x6  = 111 (antes del trap)");
    check_reg( 7, x"000000DE", "x7  = 222 (retorno de MRET)");
    check_reg(28, x"0000000B", "x28 = mcause = 11 (ecall M)");
    check_reg(29, x"0000002A", "x29 = 42  (handler ejecutado)");
    check_reg(30, x"00000010", "x30 = mepc+4 = 16");

    report "-----------------------------------------";
    if errors = 0 then
      report "TODOS LOS TESTS DE TRAPS PASARON" severity note;
    else
