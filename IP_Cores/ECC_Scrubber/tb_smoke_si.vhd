...
entity tb_smoke_si is end entity;
architecture sim of tb_smoke_si is
...
begin
  ...
  dut: entity work.ecc_soc_si
    generic map (ADDR_W=>16, DEPTH=>256, SCRUB_DEPTH=>32, IMEM_INIT=>"smoke_si.mem", DONE_WORD=>127, AXI_AW=>40)
    port map (aclk=>aclk, aresetn=>aresetn,
      ...
      irq_out=>irq);
  process
    alias pcs is << signal .tb_smoke_si.dut.dbg_pc : work.riscv_pkg.word_t >>;
  ...
  begin
    ...
    report "smoke_si fin: irq=" & std_logic'image(irq);
    wait;
  end process;
end architecture;
