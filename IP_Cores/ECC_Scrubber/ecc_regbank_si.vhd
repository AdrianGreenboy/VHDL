...
architecture rtl of ecc_regbank_si is
  constant CORE_ID : std_logic_vector(31 downto 0) := x"5C520020";

  constant OFF_ID : std_logic_vector(7 downto 0) := x"40";
  constant OFF_STATUS : std_logic_vector(7 downto 0) := x"44";
  constant OFF_CONTROL : std_logic_vector(7 downto 0) := x"48";
  constant OFF_REGION_BASE : std_logic_vector(7 downto 0) := x"4C";
  constant OFF_REGION_LEN : std_logic_vector(7 downto 0) := x"50";
  constant OFF_PERIOD : std_logic_vector(7 downto 0) := x"54";
  constant OFF_CE_COUNT : std_logic_vector(7 downto 0) := x"58";
  constant OFF_DED_COUNT : std_logic_vector(7 downto 0) := x"5C";
  constant OFF_FIRST_ADDR : std_logic_vector(7 downto 0) := x"60";
  constant OFF_FIRST_SYN : std_logic_vector(7 downto 0) := x"64";
  constant OFF_LAST_ADDR : std_logic_vector(7 downto 0) := x"68";
  constant OFF_LAST_SYN : std_logic_vector(7 downto 0) := x"6C";
  constant OFF_INJ_ADDR : std_logic_vector(7 downto 0) := x"70";
  constant OFF_INJ_MASK_LO : std_logic_vector(7 downto 0) := x"74";
  constant OFF_INJ_MASK_HI : std_logic_vector(7 downto 0) := x"78";
  constant OFF_INJ_CTRL : std_logic_vector(7 downto 0) := x"7C";
  constant OFF_SCRATCH : std_logic_vector(7 downto 0) := x"80";
  constant OFF_CE_BUMP  : std_logic_vector(7 downto 0) := x"84";
  constant OFF_DED_BUMP : std_logic_vector(7 downto 0) := x"88";
  ...
  -- contadores del barrido por tiles (auto-incremento por MMIO desde el firmware)
  signal tile_ce  : unsigned(31 downto 0) := (others => '0');
  signal tile_ded : unsigned(31 downto 0) := (others => '0');

  -- acelerador de codec MMIO
  signal codec_sel   : std_logic;
  signal codec_rdata : std_logic_vector(31 downto 0);
...
begin
  ...
  -- acelerador de codec MMIO en offsets 0x00-0x1F (addr(6)=0 y addr(12)=0)
  codec_sel <= '1' when (sel = '1' and addr(12) = '0' and
                         unsigned(addr(7 downto 0)) < 16#40#) else '0';
  u_codec : entity work.ecc_codec_mmio
    port map (clk => clk, rst => rst,
              sel => codec_sel, wr => wr, addr => addr(7 downto 0),
              wdata => wdata, rdata => codec_rdata);
      ...
      if rst = '1' then
        ...
        if sel = '1' and wr = '1' then
          case addr(7 downto 0) is
            ...
            when OFF_CE_BUMP     => tile_ce  <= tile_ce  + 1;
            when OFF_DED_BUMP    => tile_ded <= tile_ded + 1;
            when OFF_INJ_CTRL =>
              inj_ctrl <= wdata;
              if wdata(0) = '1' then
                if wdata(1) = '1' then
            ...
            when OFF_CONTROL =>
              if wdata(2) = '1' then          -- clr_counters
                tile_ce  <= (others => '0');
                tile_ded <= (others => '0');
              end if;
              if wdata(0) = '1' then
  ...
  rd_proc : process(addr, region_base, region_len, period, inj_addr, inj_mask_lo,
                    ...
                    win_sel, win_hi, win_rdata, codec_sel, codec_rdata, tile_ce, tile_ded)
  ...
  begin
    ...
    case addr(7 downto 0) is
      ...
      when OFF_CE_COUNT    => v := std_logic_vector(unsigned(f_ce)  + tile_ce);
      when OFF_DED_COUNT   => v := std_logic_vector(unsigned(f_ded) + tile_ded);
    ...
    -- acelerador de codec: offsets 0x00-0x1F (excepto ventana)
    if codec_sel = '1' then
      v := codec_rdata;
    end if;
    rdata <= v;
